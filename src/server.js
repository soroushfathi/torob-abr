...
function secretEqual(a,b){const x=Buffer.from(a||''),y=Buffer.from(b||'');return x.length===y.length&&timingSafeEqual(x,y);}
let attempts=[];
app.post('/api/session',async(req,res)=>{
 attempts=attempts.filter(t=>t>Date.now()-60000);
 if(attempts.length>=20)return res.status(429).json({error:'Try again in one minute'});attempts.push(Date.now());
 if(!secretEqual(req.body.token,process.env.LOCAL_ACCESS_TOKEN))return res.status(401).json({error:'کلید ورود معتبر نیست'});
 const token=randomBytes(32).toString('hex'),id=createHash('sha256').update(token).digest('hex');
 await pool.query('INSERT INTO app.sessions(id,source) VALUES($1,$2)',[id,req.body.source==='verification'?'verification':'user']);
...
app.use('/api',async(req,res,next)=>{
 const token=req.headers.cookie?.split(';').map(x=>x.trim()).find(x=>x.startsWith('torob_session='))?.slice(14);
 if(!token)return res.status(401).json({error:'ورود لازم است'});
 const id=createHash('sha256').update(token).digest('hex'),r=await pool.query('SELECT id,source FROM app.sessions WHERE id=$1 AND expires_at>now()',[id]);
 if(!r.rowCount)return res.status(401).json({error:'نشست منقضی شده'});req.session=r.rows[0];next();
});
const requirements=z.object({description:z.string().max(4000).default(''),stack:z.string().max(80).default('Next.js'),database:z.enum(['PostgreSQL','MySQL','unknown']).default('PostgreSQL'),region:z.enum(['Iran','any','unknown']).default('Iran'),operations:z.enum(['small-team','experienced','unknown']).default('small-team'),budget:z.string().max(60).default('unknown'),storage:z.string().max(120).default('unknown'),traffic:z.string().max(120).default('unknown')});
...
app.get('/api/me',(req,res)=>res.json({source:req.session.source}));
app.get('/api/catalog',(req,res)=>res.json(catalog));
app.get('/api/projects',async(req,res)=>res.json((await pool.query('SELECT * FROM app.projects WHERE owner=$1 ORDER BY created_at DESC',[req.session.id])).rows));
...
app.post('/api/projects/:id/recommend',async(req,res)=>{
 await owned(req);const input=requirements.parse(req.body),result=recommend(input),db=await pool.connect();
 counters.ai++;counters.fallback++;
 try{await db.query('BEGIN');await db.query('UPDATE app.projects SET requirements=$1,recommendation=$2,selected=NULL WHERE id=$3',[input,result,req.params.id]);await event(db,req.params.id,'intake_completed',req.session);if(result.eligibleCount>0)await event(db,req.params.id,'recommendation_succeeded',req.session);else await event(db,req.params.id,'no_eligible_option',req.session);await db.query('COMMIT');res.json(result);}catch(e){await db.query('ROLLBACK');throw e;}finally{db.release();}
});
app.post('/api/projects/:id/events',async(req,res)=>{
 const p=await owned(req),input=z.object({name:z.enum(['comparison_viewed','checklist_viewed','provider_clicked','feedback_useful','feedback_not_useful']),provider:z.enum(['liara','hetzner','render']).optional()}).parse(req.body);
 ...
 await event(pool,p.id,input.name,req.session,input.name==='provider_clicked'?p.selected:'');res.json({ok:true});
});
app.post('/api/projects/:id/select',async(req,res)=>{
 const p=await owned(req),id=z.enum(['liara','hetzner','render']).parse(req.body.id),o=p.recommendation?.options.find(x=>x.id===id);
 if(!o||o.status==='ineligible')return res.status(409).json({error:'این گزینه شرایط لازم را ندارد'});
 const db=await pool.connect();try{await db.query('BEGIN');await db.query('UPDATE app.projects SET selected=$1 WHERE id=$2',[id,p.id]);await event(db,p.id,'plan_selected',req.session,id);await db.query('COMMIT');res.json({option:o,confirmedEligible:o.status==='eligible'});}catch(e){await db.query('ROLLBACK');throw e;}finally{db.release();}
});
...
async function pushTelemetry(){
 ...
 const values={torob_http_requests_total:counters.requests,torob_http_errors_total:counters.errors,torob_http_duration_seconds_sum:counters.duration,torob_http_duration_seconds_count:counters.requests,torob_db_up:dbUp,torob_process_rss_bytes:process.memoryUsage().rss,torob_process_cpu_ratio:ratio,torob_ai_requests_total:counters.ai,torob_ai_fallback_total:counters.fallback,torob_ai_failures_total:0,torob_ai_tokens_total:0,torob_ai_duration_seconds_sum:0,torob_ai_duration_seconds_count:0};
