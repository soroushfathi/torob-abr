...
export function seedTariffs() {
 ...
 for(const o of catalog.offers.filter(o=>o.model==='IaaS')) plans.push({
  ...
  egressPerGB:o.egressPerGB??null,trafficGB:o.trafficGB??null,availability:o.availability??'unknown',
 ...
 return plans.map(p=>({...p,pricingModel:'fixed_plan',verificationStatus:'manual_evidence',unitRate:null,taxStatus:'unknown',freeQuota:null,usageTiers:null,minimumOrder:null}));
}
export async function loadCatalog(pool) {
 ...
 return {...catalog,version:[catalog.version,...versions.map(v=>`${v.provider_id}:${v.id}`)].join('|'),
